...
const chartType = process.argv[2] || 'top-songs';
const data = JSON.stringify({ chartType: chartType, limit: 5 });

const options = {
  hostname: 'szfxnzsapojuemltjghb.supabase.co',
  port: 443,
  path: '/functions/v1/get-apple-charts',
  method: 'POST',
  headers: {
...
console.log('Testing get-apple-charts edge function...');
    ...
    try {
      ...
      if (json.error) {
        ...
        json.tracks.forEach((t, i) => {
          ...
          if (t.previewUrl) {
            console.log('     Preview: ' + t.previewUrl.substring(0, 60) + '...');
          }
        });
      } else {
